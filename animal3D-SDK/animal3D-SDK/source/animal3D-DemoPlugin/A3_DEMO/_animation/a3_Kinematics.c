...
a3i32 a3kinematicsSolveForwardPartial(const a3_HierarchyState *hierarchyState, const a3ui32 firstIndex, const a3ui32 nodeCount)
{
	...
	//	a3i32 parentIndex;
		...
		for (i = firstIndex; i < end; ++i)
		{
			...
			if (hierarchyState->poseGroup->hierarchy->nodes->parentIndex != -1)
...
a3i32 a3kinematicsSolveInversePartial(const a3_HierarchyState *hierarchyState, const a3ui32 firstIndex, const a3ui32 nodeCount)
{
	...
	//	a3i32 parentIndex;
		...
		for (i = firstIndex; i < end; ++i)
		{
			...
			if (hierarchyState->poseGroup->hierarchy->nodes->parentIndex != -1)
			{
				a3real4x4Product(hierarchyState->localSpace->transform[i].m, hierarchyState->objectSpaceInverse->transform[hierarchyState->poseGroup->hierarchy->nodes[i].parentIndex].m, hierarchyState->objectSpace->transform[i].m);
			}
			else
			{
